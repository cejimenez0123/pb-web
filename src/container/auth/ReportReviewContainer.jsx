...
((state) => state.blocks || {});
  ...
  const removeGroup = (reportedProfileId) => {
    showAlert({message:"banned",type:AlertType.success})
    
    closeDialog();
  };
  console.log(blockEvents)
  const handleBan = ({reportedProfileId, reportIds=[],blockIds = []}) => {
    dispatch(banUser({ reportedProfileId, reportIds,blockIds }))
      .unwrap()
  ...
  const openUserDialog = (group) => {
    ...
    openDialog({
      ...
      agree: () => handleBan({reportedProfileId:group.reportedProfileId, reportIds:allReportIds}),
                ...
                <div className="flex flex-col gap-2">
                  {blockEvents.map((event) => (
                    ...
                    >
                      ...
                      <span onClick={()=>handleBan({reportedProfileId: event.blockedProfileId,blockIds:[event.id]})}className="text-sm btn text-soft/60">Ban</span>
                    </button>
                  ))}
                </div>
              )}
