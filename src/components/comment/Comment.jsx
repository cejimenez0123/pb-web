...
export default function Comment({ page, comment, level = 0 }) {
  ...
  useLayoutEffect(() => {
    // Children are already nested on the comment from the API,
    // but also check flat list for any newly added replies
    const fromChildren = comment.children ?? [];
    const fromFlat = allComments.filter((c) => c.parentId === comment.id);

    // Merge, deduplicating by id
    const seen = new Set();
    const merged = [...fromChildren, ...fromFlat].filter((c) => {
