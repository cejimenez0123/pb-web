...
//     if (currentProfile) {
//       dispatch(getRecommendedCollectionsProfile()).finally(() => setIsLoading(false));
//     } else{
//       collection && dispatch(getRecommendedCollections({ colId: collection.id })).finally(() => setIsLoading(false));
//     }
//   }, [currentProfile,collection, dispatch]);
//   console.log("TOTM"<items)
//   return (
//     <div className="min-h-[14rem] mt-4 pb-[10rem] bg-cream">
//       {/* Header */}
//       <h3 className="text-emerald-900 font-extrabold ml-16 lora-bold mb-4 text-2xl">
//         Explore
//       </h3>
...
//         {items?.length?
//         items?.map((item) => <BookListItem key={item.id} book={item} />):
        
//          [...Array(4)].map((_, i) => (
//               <div
//                 key={i}
//                 className="min-w-[12rem] h-[12rem] bg-white rounded-xl shadow-md p-3 flex flex-col justify-between animate-pulse"
//               >
