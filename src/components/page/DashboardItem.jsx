...
//               sendGAEvent("Story Visible",`Story Visible `+page.title
//            )
...
//    return <span className={"flex-row flex justify-between w-[96vw]  md:w-page px-1 rounded-t-lg  pt-2 pb-1"}>      
//     {isNotPhone&& !isGrid?<ProfileCircle isGrid={isGrid} profile={page.author}/> :null}     
// <h6 className={`${!isGrid?"text-emerald-800":" text-white"}
//      ml-1 pr-2 
...
//     }
//     const bookmarkBtn =()=>{
//         return isGrid ?<div className='w-[96%] mx-auto  py-2 my-auto flex flex-row justify-between  text-white '>
//             {!isPhone?<><ProfileCircle isGrid={isGrid} profile={page.author}/></>:null}
//         {isNotPhone?<span className='bg-transparent flex flex-row '>
//             <h6 className={`text-white max-w-[14em] min-w-[8em]  ${isPhone?"px-1":"ml-1 pr-1 text-right"} no-underline text-ellipsis  whitespace-nowrap overflow-hidden  my-auto text-[0.9rem]`}
//     onClick={()=>{
//         navigate(Paths.page.createRoute(page.id))
//     }}

// >{` `+page.title.length>0?page.title:""}</h6><img onClick={handleBookmark}
// className='text-white' src={bookmarked?bookmarkfill:bookmarkoutline}/></span>:null}
...
//         <div  ref={ref} className={`${isGrid?isNotPhone?"bg-emerald-700  max-w-grid":"my-2  max-w-grid-mobile  md:max-h-[24em] bg-emerald-700":"bg-emerald-100 max-w-grid "} shadow-sm   rounded-lg  overflow-hidden max-w-[100%] ${!isGrid?isPhone?" bg-emerald-100":" bg-emerald-100 rounded-lg overflow-clip min-h-56   ":"relative overflow-clip shadow-md md:w-page  my-2"}   `}>
             
//               {!isGrid?header():null}
//         {page.description?<div className='min-h-16 p-2 text-emerald-700'>
//             {page.needsFeedback?<label className='text-emerald-800'>Feedback Request:</label>:null}
//             <h6 className={`${isGrid?isNotPhone?"text-white md:max-w-grid":"text-white":"text-emerald-800"} open-sans-medium text-left `}>
//                 {page.description}
//             </h6>
//         </div>:null}
...
//           <div onClick={()=>{
//          navigate(Paths.page.createRoute(page.id))
//         }}  className={isGrid?isPhone?'w-grid-mobile md:w-grid-content rounded-lg pl-1  pt-1':"":" rounded-lg"}>
//       {/* <div > */}
...
//                 {buttonRow()}
//                {isGrid?
//                  <div className='max-w-[100%] flex flex-row pt-2 justify-between  py-1  rounded-b-lg bottom-0'>
//                 {/* {header()} */}
//                 {bookmarkBtn()} </div>  
//               :null} 
//               </div>
...
className='  bg-emerald-700 flex grow flex-1/3 '> <img  className="mx-auto my-auto" src={addCircle}/></div>}
    ...
    if(page){
        ...
        <div className={`shadow-sm ${isGrid?"bg-emerald-700 rounded-lg min-h-56   ":"bg-emerald-50 rounded-t-lg md:w-page w-[96vw]"}   `}>
               {!isGrid?header():null}
        {page.description && page.description.length>0?<div className='h-12 pt-4 p-2'>
            {page.needsFeedback?<label className='text-emerald-800'>Feedback Request:</label>:null}
            <h6 className={`${!isGrid?"text-emerald-800":isPhone?"text-white max-h-8 overflow-scroll":"text-white "} p-2 open-sans-medium text-left `}>
                {page.description}
            </h6>
        </div>:null}
       
             
          <div className={isGrid?isPhone?"h-36 rounded-lg overflow-clip":' rounded-lg flex justify-between flex-col h-[100%]  pt-1':"rounded-lg"}>
      <div onClick={()=>{
         navigate(Paths.page.createRoute(page.id))
        }} >
          <PageDataElement  isGrid={isGrid} page={page}/>
